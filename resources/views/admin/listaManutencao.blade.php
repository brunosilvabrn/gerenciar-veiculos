...
@section('content')
    <main class="h-full overflow-y-auto">
        <div class="container grid px-6 mx-auto">
            <h2 class="my-6 text-2xl text-center font-semibold text-gray-700 dark:text-gray-200">Lista Manutenções</h2>
                ...
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        >
                            ...
                            <th class="w-1/6 px-4 py-3">Actions</th>
                        </tr>
                        </thead>
                        <tbody
                                    ...
                                    <div class="flex items-center text-sm">
                                        <div>
                                            <p class="text-lg text-gray-600 dark:text-gray-400 font-medium">
                                                {{ $manutencao->carro->modelo->name }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $manutencao->carro->marca->name }}
                                </td>
                                <td class="w-2/6 px-4 py-3 text-sm">
                                    {{ $manutencao->descricao }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $manutencao->created_at)->format('d/m/Y') }}
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        <a href="{{ route('maintenance.finalize', $manutencao->id) }}"
                                           class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                            Finalizar
                                        </a>

                                        <a href="{{ route('maintenance.show', $manutencao->id) }}"
                                        ...
                                        <a href="{{ route('maintenance.delete', $manutencao->id) }}"
