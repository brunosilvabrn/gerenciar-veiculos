...
@section('content')
    <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
            <h2 class="my-6 text-2xl text-center font-semibold text-gray-700 dark:text-gray-200">Editar veiculo</h2>
            <form method="POST" action="{{ route('vehicle.edit', $veiculo->id) }}" class="flex flex-wrap justify-evenly px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                @csrf
                ...
                <label class="block text-sm p-2">
                    ...
                    <div class="relative">
                        <select style="width: 130px" name="marca" id="marcas" onclick="clickModelo()" disabled class="block w-50 mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" id="grid-state">
                            <option value="{{ $veiculo->marca->id }}">{{ $veiculo->marca->name }}</option>
                        </select>
                ...
                <label class="block text-sm p-2">
                    ...
                    <div class="relative">
                        <select name="modelo" style="width: 130px" id="selectOptionModelo" disabled class="block w-50 mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" id="grid-state">
                            <option id="selectOptionModelo" value="{{ $veiculo->modelo->id }}">{{ $veiculo->modelo->name }}</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                        </div>
                ...
                <label class="block text-sm p-2">
                    ...
                    <input name="placa" value="{{ $veiculo->placa }}"
                ...
                <label class="block text-sm p-2">
                    ...
                    <input name="ano" value="{{ $veiculo->ano }}"
                           class="block w-50 mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                           placeholder="2022"
                    />
                </label>
