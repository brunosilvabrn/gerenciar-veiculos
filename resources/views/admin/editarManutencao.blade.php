...
@section('content')
    <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
            ...
            <form method="POST" action="{{ route('maintenance.edit', $manutencao->id) }}"
                  class="flex flex-wrap justify-between px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                @csrf
                ...
                <label class="block text-sm p-2">
                    ...
                    <div class="relative">
                        <select name="modelo" style="width: 150px"
                            ...
                            @foreach($modelos as $modelo)
                                <option
                                    @if($modelo->id === $manutencao->modelo_id)
                                        selected
                                    @endif
                                    value="[{{ $modelo->id }}">{{ $modelo->marca->name }}
                                    - {{ $modelo->modelo->name}}</option>
                            @endforeach
                ...
                <label class="block text-sm p-2">
                    ...
                    <input type="date" name="data"
                           value="{{ Carbon\Carbon::createFromFormat('Y-m-d H:m:s', $manutencao->data)->format('Y-m-d')  }}"
                           class="block w-50 mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                    />
                </label>
